...
command+=" --clipping_type=\"no_clip\""
command+=" --clipping_level=2.0"
command+=" --clip_sstm_L=10.0"

# Learning Rate Loop
for method in \"zo_clipped_sgd\" \"zo_clipped_sstm\"; do
    for learning_rate in 1e-3; do
        for scheduler in \"constant\"; do
            full_command="$command --trainer=$method --learning_rate=$learning_rate --scheduler=$scheduler"
            eval "$full_command"
        done
    done
done
