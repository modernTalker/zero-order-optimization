    ...
    def step(self, closure):
        ...
        if args.module_wise_perturbation:
            ...
            if args.coordinate_perturbation:
                return torch.sign(self.zo_step_with_module_wise_perturbation_coordinate(closure))
            else:
                return torch.sign(self.zo_step_with_module_wise_perturbation(closure))
        elif args.q == 1:
            return torch.sign(self.zo_step(closure))
        elif args.q > 1:
            return torch.sign(self.zo_step_v1(closure))
        else:
