...
class ZO_SignSGD(ZeroOrderOptimizer):
    ...
    def step(self, model, inputs):
        ...
        if args.module_wise_perturbation:
            ...
            if args.coordinate_perturbation:
                return torch.sign(self.zo_step_with_module_wise_perturbation_coordinate(model, inputs))
            else:
                return torch.sign(self.zo_step_with_module_wise_perturbation(model, inputs))
        elif args.q == 1:
            return torch.sign(self.zo_step(model, inputs))
        elif args.q > 1:
            return torch.sign(self.zo_step_v1(model, inputs))
        else:
