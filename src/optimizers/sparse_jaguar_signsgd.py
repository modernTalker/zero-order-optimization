...
class Sparse_Jaguar_SignSGD(ZeroOrderOptimizer):
    def __init__(self, 
        ...
        # Lazy init states in __init__ for stability
    ...
    def step(self, closure=None):
        ...
        # 1. Generate single seed for this step
        ...
        # 3. Backward pass: Reset seed -> same z -> w - εz
        self.generator.manual_seed(self.zo_random_seed)
        self._sparse_indices_perturb(scaling_factor=-2.0, params_ratio=self.params_ratio)
        loss2 = closure()

        # 4. Restore original weights: Reset seed -> same z -> w
        self.generator.manual_seed(self.zo_random_seed)
        self._sparse_indices_perturb(scaling_factor=1.0, params_ratio=self.params_ratio)

        # 5. Compute scalar directional derivative
        grad_scalar = self.grad_approx(loss1, loss2, perturbation_mode="two_side")

        # 6. Select subset for parameter update (CONTINUES RNG stream naturally)
        ...
        for group in self.param_groups:
            ...
            for param in group['params']:           
                if id(param) not in selected_param_ids:
                    continue  # Skip unselected params (momentum preserved)
                    
                state = self.state[param]
                device = param.device
                
                # Project scalar derivative onto random direction z
